...
@app.route('/model_governance')
def model_governance():
    return render_template('model_governance.html')

@app.route('/llm_mesh')
def llm_mesh():
    return render_template('llm_mesh.html')  # Or render_template if using templates

@app.route('/ml_integration')
def ml_integration():
    return render_template('ml_integration.html')
...
@app.route('/ai_data_enrichment')
def ai_data_enrichment():
    return render_template('ai_data_enrichment.html')
...
def generate_code():
    ...
    # For demo purposes, we'll return example code
    if language == 'python':
        if task == 'generate':
            code = f"""# Python code generated based on: {instructions}
import pandas as pd
...
if __name__ == "__main__":
    result = etl_process('input.csv', 'output.csv')
    print(f"Processed {len(result)} records")"""
        elif task == 'explain':
            code = f"""# Explanation of the provided Python code:

The code performs the following operations:
1. Loads data from a CSV file
2. Cleans the data by removing missing values
3. Performs a calculation to create a new column
4. Saves the results to a new file

Key functions used:
- read_csv() - for loading data
- dropna() - for handling missing values
- Basic arithmetic operations for calculations
- to_csv() - for saving results"""
        elif task == 'optimize':
            code = f"""# Optimized Python code based on: {instructions}
import pandas as pd

def optimized_etl(input_file, output_file):
    # Load data with optimized parameters
    df = pd.read_csv(input_file, engine='c', memory_map=True)
    
    # More efficient cleaning
    df = df.dropna(subset=['value1', 'value2'])
    
    # Vectorized calculation
    df['calculated'] = df['value1'].mul(df['value2'])
    
    # Efficient saving
    df.to_csv(output_file, index=False, mode='w')
    return df"""
        elif task == 'debug':
            code = f"""# Debugged Python code:
{existing_code}

# Issues fixed:
1. Missing import statement added
2. Fixed column name typo
3. Added error handling"""
        elif task == 'convert':
            code = f"""# Python code converted from other language:
...
def etl_process(input_file, output_file):
    ...
    pass"""
        ...
        if task == 'generate':
            code = f"""# R code generated based on: {instructions}
etl_process <- function(input_file, output_file) {{
    # Load data
    ...
    # Example transformation
    if ("value1" %in% colnames(df) && "value2" %in% colnames(df)) {{
        df$calculated <- df$value1 * df$value2
    }}
...
# Example usage
result <- etl_process("input.csv", "output.csv")
print(paste("Processed", nrow(result), "records"))"""
        elif task == 'explain':
            code = f"""# Explanation of the provided R code:
...
Key functions used:
- read.csv() - for loading data
- na.omit() - for handling missing values
- Basic arithmetic operations for calculations
- write.csv() - for saving results"""
        elif task == 'optimize':
            code = f"""# Optimized R code based on: {instructions}
library(data.table)

optimized_etl <- function(input_file, output_file) {{
    # Faster data loading
    dt <- fread(input_file)
    
    # More efficient cleaning
    dt <- na.omit(dt, cols=c("value1", "value2"))
    
    # Efficient calculation
    dt[, calculated := value1 * value2]
    ...
    return(dt)
            ...
            code = f"""# Debugged R code:
{existing_code}

# Issues fixed:
1. Missing library statements added
2. Fixed column name typo
3. Added error handling"""
        elif task == 'convert':
            code = f"""# R code converted from other language:
# Original task: {instructions}
...
@app.route('/ai_documentation')
def ai_documentation():
    return render_template('ai_documentation.html')


@app.route('/nlq_etl_design')
def nlq_etl_design():
    return render_template('nlq_etl_design.html')



@app.route('/predictive_scoring')
def predictive_scoring():
    return render_template('predictive_scoring.html')


from flask import Flask, render_template, request, session, redirect, url_for, jsonify
app.secret_key = os.urandom(24) 
@app.route('/code_friendly_modeling')
def code_friendly_modeling():
    # Get the data from session (passed from the main interface)
    data = {
        'data': session.get('current_data', []),
        'columns': session.get('current_columns', []),
        'filename': session.get('current_filename', '')
    }
    
    # Store in sessionStorage to pass to the new tab
    session['codeFriendlyData'] = json.dumps(data)
    
    return render_template('code_friendly_modeling.html')
...
@app.route('/analyze_text_data', methods=['POST'])
def analyze_text_data():
    try:
        ...
        if not text_columns:
            return jsonify({"status": "error", "message": "No text columns found in the data"}), 400
        
        # Perform text analysis (in a real implementation, this would use proper NLP libraries)
        ...
        sentiment_samples = []
        for col in text_columns[:2]:  # Limit to first 2 text columns for demo
            sample_texts = df[col].dropna().sample(min(5, len(df))).tolist()
        ...
        # Column-wise sentiment distribution
        column_sentiments = []
        for col in text_columns:
            column_sentiments.append({
                "column": col,
        ...
        word_frequencies = []
        for word in ["customer", "service", "product", "quality", "experience", "price", "value", "support", "issue", "time"]:
        ...
        topic_modeling = {
            "topics": [
                ...
                {
                    "weight": 0.25,
                    "keywords": ["product", "quality", "feature", "design"]
                },
                {
                    "weight": 0.20,
                    "keywords": ["price", "value", "cost", "expensive"]
                },
                {
                    "weight": 0.15,
                    "keywords": ["delivery", "time", "shipping", "fast"]
                },
                {
                    "weight": 0.05,
                    "keywords": ["website", "app", "interface", "mobile"]
                }
            ]
        }
        ...
        named_entities = {
            "entity_types": [
                {"type": "Person", "count": random.randint(50, 100)},
                {"type": "Organization", "count": random.randint(30, 80)},
                {"type": "Location", "count": random.randint(20, 60)},
                {"type": "Date", "count": random.randint(40, 90)},
                {"type": "Product", "count": random.randint(10, 40)}
            ],
            "entities": [
                {"type": "Person", "entity": "John Smith", "count": random.randint(5, 15)},
                {"type": "Person", "entity": "Sarah Johnson", "count": random.randint(3, 10)},
                {"type": "Organization", "entity": "Acme Corp", "count": random.randint(8, 20)},
                {"type": "Location", "entity": "New York", "count": random.randint(5, 12)},
                {"type": "Product", "entity": "Model X", "count": random.randint(2, 8)}
            ]
        }
        ...
        response_data = {
            ...
            "column_sentiments": column_sentiments,
        ...
        logger.error(f"Error analyzing text data: {str(e)}")
        return jsonify({
            "status": "error",
            "message": f"Error analyzing text data: {str(e)}"
        }), 500
...
#         # Get parameters
#         lat_field = data.get('latitude_field')
#         lng_field = data.get('longitude_field')
#         analysis_type = data.get('analysis_type', 'heatmap')
        
#         # Validate
#         if not lat_field or not lng_field:
#             return jsonify({'status': 'error', 'message': 'Latitude and longitude fields are required'})
        
#         # Clean data
#         df = df.dropna(subset=[lat_field, lng_field])
#         df[lat_field] = pd.to_numeric(df[lat_field], errors='coerce')
#         df[lng_field] = pd.to_numeric(df[lng_field], errors='coerce')
#         df = df.dropna(subset=[lat_field, lng_field])
...
#             'point_count': len(df),
#             'bounds': {
#                 'min_lat': df[lat_field].min(),
#                 'max_lat': df[lat_field].max(),
#                 'min_lng': df[lng_field].min(),
#                 'max_lng': df[lng_field].max()
#             },
#             'hotspots': generate_hotspots(df, lat_field, lng_field),
#             'stats': generate_geospatial_stats(df, lat_field, lng_field),
#             'llm_insights': generate_llm_insights(df, analysis_type)
#         }
...
# def generate_hotspots(df, lat_field, lng_field):
#     # Simple hotspot detection - in a real app you might use DBSCAN or similar
#     hotspots = []
#     for _ in range(3):  # Generate 3 fake hotspots
#         center_lat = random.uniform(df[lat_field].min(), df[lat_field].max())
#         center_lng = random.uniform(df[lng_field].min(), df[lng_field].max())
...
#             'radius': radius,
#             'intensity': intensity,
#             'point_count': random.randint(10, 100)
#         })
    
#     return hotspots

# def generate_geospatial_stats(df, lat_field, lng_field):
#     # Calculate basic spatial statistics
#     stats = {
#         'mean_center': [df[lat_field].mean(), df[lng_field].mean()],
#         'std_dev': {
#             'lat': df[lat_field].std(),
#             'lng': df[lng_field].std()
#         },
#         'quadrat_count': 4,  # In real app, calculate based on grid
#         'nearest_neighbor': random.uniform(0.5, 1.5)  # Fake NNI
#     }
    
#     return stats

# def generate_llm_insights(df, analysis_type):
#     # Simulate LLM insights - in a real app you'd call an LLM API
#     insights = [
#         f"The {analysis_type} analysis reveals spatial patterns in the data.",
#         "Several clusters of high activity are visible in urban areas.",
#         "The distribution appears to follow expected geographic trends.",
#         "Recommend further investigation of outliers in the dataset."
#     ]
    
#     return {
#         'summary': random.choice(insights),
#         'recommendations': [
#             "Consider temporal analysis to identify patterns over time",
#             "Compare with demographic data for deeper insights",
#             "Validate findings with ground truth data where possible"
#         ]
#     }

# @app.route('/geospatial/llm', methods=['POST'])
# def geospatial_llm():
#     try:
#         data = request.get_json()
...
#         responses = {
#             'heatmap': [
#                 "The heatmap shows concentrations of activity in specific areas.",
#                 "Heat intensity varies significantly across the geographic region.",
#                 "Several hotspots are visible in the heatmap visualization."
#             ],
#             'clustering': [
#                 "Three distinct clusters are evident in the data.",
#                 "The clustering algorithm has identified natural groupings.",
#                 "Cluster sizes indicate varying levels of activity concentration."
#             ],
#             'hotspots': [
#                 "Hotspot analysis reveals statistically significant areas.",
#                 "Primary and secondary hotspots are clearly identifiable.",
#                 "The hotspots correlate with known geographic features."
#             ],
#             'density': [
#                 "Density patterns show urban versus rural differences.",
#                 "High density corridors are visible along transportation routes.",
#                 "The density gradient reveals important spatial patterns."
#             ]
#         }
        
#         answer = random.choice(responses.get(analysis_type, [
#             "The spatial analysis reveals interesting patterns.",
#             "Geographic distribution shows non-random clustering.",
#             "Further investigation of spatial patterns is recommended."
#         ]))
...
#             'answer': answer,
#             'follow_up_questions': [
#                 "How do these patterns vary over time?",
#                 "What demographic factors might explain these patterns?",
#                 "Are there any environmental correlations with these results?"
#             ]
#         })
...
def generate_llm_insights(df):
    """Generate simulated LLM insights about the data"""
    # In a real implementation, this would call an LLM API
    insights = [
        "The dataset appears to be well-structured with consistent data types across columns.",
        "Numeric columns show good distribution with no extreme outliers detected.",
        "Text columns contain mostly unique values, suggesting they may be identifiers or free-form text.",
        "Date columns are properly formatted and within expected ranges."
    ]
    
    suggestions = [
        "Consider normalizing the numeric columns to bring them to a common scale.",
        "For text columns with high cardinality, consider dimensionality reduction techniques.",
        "Check for potential relationships between key numeric columns using correlation analysis."
    ]
...
def execute_code():
    ...
    # For demo purposes, we'll return example output
    if language == 'python':
        output = "Python code executed successfully!\nProcessed 1245 records"
    elif language == 'r':
